...
export class TitleBlock extends BaseBlock {
  private readonly titleSelector: string = 'xpath=//div[@class="blockTitle" or contains(@class, "blockTitle ")]';

  public async isPresentOnPage(): Promise<boolean> {
    return this.parentElement.locator(this.titleSelector).isVisible();
  }
  ...
  public async verifyBlock(expectedStructure: any): Promise<void> {
    ...
    if (expectedStructure.content) {
      const titleContent = await this.parentElement.locator(this.titleSelector).innerText();
